...
from src.config import (
    MODELS_DIR, LBPH_MODEL_PATH, LABELS_PATH, LOGS_DIR,
...
def append_logs(row):
    ...
    with open(ACCESS_LOG_CSV, "a", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        if not exists:
            w.writerow(["timestamp", "persona", "resultado", "confianza",
                        "latencia_ms", "fps", "audio_path", "record_hash"])
        w.writerow([row[k] for k in ["timestamp","persona","resultado","confianza",
                                     "latencia_ms","fps","audio_path","record_hash"]])
...
def main():
    ...
    while True:
        ...
        faces = face_cascade.detectMultiScale(
            gray, DETECT_SCALE, DETECT_NEIGH, minSize=DETECT_MINSZ
        )
        ...
        for (x, y, w, h) in faces:
            ...
            if result == "ACCESO_CONCEDIDO":
                log_access_to_excel(
                    name, result, confidence, image_path=f"image_{name}.png"
                )
            ...
            cv2.putText(frame,
                        f"conf:{row['confianza']}  fps:{row['fps']}",
                        (x, y+h+20),
